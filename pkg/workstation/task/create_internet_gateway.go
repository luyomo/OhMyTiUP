...
func (c *CreateInternetGateway) Execute(ctx context.Context) error {
	...
	stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-internet-gateways --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisamplews\"", false)
	if err != nil {
	...
	command := fmt.Sprintf("aws ec2 create-internet-gateway --tag-specifications \"ResourceType=internet-gateway,Tags=[{Key=Name,Value=%s}]\"", "tisamplews")
	...
	command = fmt.Sprintf("aws ec2 create-route --route-table-id %s --destination-cidr-block 0.0.0.0/0 --gateway-id %s", clusterInfo.routeTableId, newInternetGateway.InternetGateway.InternetGatewayId)
	fmt.Printf("The comamnd is <%s> \n\n\n", command)
	stdout, stderr, err = local.Execute(ctx, command, false)
	if err != nil {
		fmt.Printf("The error here is <%#v> \n\n", err)
