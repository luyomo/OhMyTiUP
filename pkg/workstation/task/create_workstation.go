...
func (c *CreateWorkstation) Execute(ctx context.Context) error {
	...
	stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-instances --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisamplews\" \"Name=instance-state-code,Values=0,16,32,64,80\"", false)
	if err != nil {
	...
	for i := 1; i <= 10; i++ {
		stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-instances --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisamplews\" \"Name=instance-state-code,Values=0,16\"", false)
		if err != nil {
			fmt.Printf("The error here is <%#v> \n\n", err)
			fmt.Printf("----------\n\n")
			fmt.Printf("The error here is <%s> \n\n", string(stderr))
