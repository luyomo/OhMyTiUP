...
func (c *CreateSecurityGroup) Execute(ctx context.Context) error {
	...
	stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-security-groups --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisamplews\"", false)
	if err != nil {
	...
	command := fmt.Sprintf("aws ec2 create-security-group --group-name tisamplews --vpc-id %s --description tisamplews --tag-specifications \"ResourceType=security-group,Tags=[{Key=Name,Value=tisamplews}]\"", clusterInfo.vpcInfo.VpcId)
	...
	command = fmt.Sprintf("aws ec2 authorize-security-group-ingress --group-id %s --protocol tcp --port 22 --cidr 0.0.0.0/0", clusterInfo.securityGroupId)
	fmt.Printf("The comamnd is <%s> \n\n\n", command)
	stdout, stderr, err = local.Execute(ctx, command, false)
	if err != nil {
		fmt.Printf("The error here is <%#v> \n\n", err)
