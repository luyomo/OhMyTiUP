...
func (c *CreateRouteTable) Execute(ctx context.Context) error {
	...
	stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-route-tables --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisamplews\"", false)
	if err != nil {
	...
	command := fmt.Sprintf("aws ec2 create-route-table --vpc-id %s --tag-specifications \"ResourceType=route-table,Tags=[{Key=Name,Value=tisamplews}]\"", clusterInfo.vpcInfo.VpcId)
