...
func (c *CreateVpc) Execute(ctx context.Context) error {
	...
	stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-vpcs --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisamplews\"", false)
	if err != nil {
	...
	stdout, stderr, err = local.Execute(ctx, "aws ec2 create-vpc --cidr-block 172.81.0.0/16 --tag-specifications \"ResourceType=vpc,Tags=[{Key=Name,Value=tisamplews}]\"", false)
	if err != nil {
		fmt.Printf("The error here is <%#v> \n\n", err)
		fmt.Printf("----------\n\n")
		fmt.Printf("The error here is <%s> \n\n", string(stderr))
