...
func (c *CreateRouteTable) Execute(ctx context.Context) error {
	...
	stdout, stderr, err := local.Execute(ctx, "aws ec2 describe-route-tables --filters \"Name=tag-key,Values=Name\" \"Name=tag-value,Values=tisampletest\"", false)
	if err != nil {
	...
	command := fmt.Sprintf("aws ec2 create-route-table --vpc-id %s --tag-specifications \"ResourceType=route-table,Tags=[{Key=Name,Value=tisampletest}]\"", clusterInfo.vpcInfo.VpcId)
