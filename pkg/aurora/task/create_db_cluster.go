...
func (c *CreateDBCluster) Execute(ctx context.Context) error {
	...
	command := fmt.Sprintf("aws rds describe-db-clusters --db-cluster-identifier '%s'", "tisampletest")
	stdout, stderr, err := local.Execute(ctx, command, false)
	if err != nil {
		if strings.Contains(string(stderr), fmt.Sprintf("DBCluster %s not found", "tisampletest")) {
		...
		fmt.Printf("The DB Cluster has been created\n\n\n")
		return nil
	}

	fmt.Printf("The DB cluster  <%s> \n\n\n", string(stdout))

	command = fmt.Sprintf("aws rds create-db-cluster --db-cluster-identifier %s --engine aurora-mysql --engine-version 5.7.12 --master-username master --master-user-password 1234Abcd --db-subnet-group-name %s --db-cluster-parameter-group-name cluster-params-%s --vpc-security-group-ids %s", "tisampletest", "tisampletest", "tisampletest", clusterInfo.securityGroupId)
