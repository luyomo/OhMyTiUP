...
func (c *ScaleTiDB) Execute(ctx context.Context) error {
	...
	for _, reservation := range c.oldInstances.Reservations {
		for _, instance := range reservation.Instances {
			for _, tag := range instance.Tags {
				if tag["Key"] == "Component" && tag["Value"] == "pd" {
					pdMap[instance.PrivateIpAddress] = true
					break
				}
				if tag["Key"] == "Component" && tag["Value"] == "tidb" {
					tidbMap[instance.PrivateIpAddress] = true
					break
				}
				if tag["Key"] == "Component" && tag["Value"] == "tikv" {
					tikvMap[instance.PrivateIpAddress] = true
					break
				}
				if tag["Key"] == "Component" && tag["Value"] == "ticdc" {
					ticdcMap[instance.PrivateIpAddress] = true
					break
				}
				if tag["Key"] == "Component" && tag["Value"] == "dm" {
					dmMap[instance.PrivateIpAddress] = true
					break
				}
			}
		}
	...
	if len(tplData.TiCDC) > 0 {
		buffer.WriteString("ticdc_servers:")
	...
	if err := os.WriteFile("/tmp/scale.yaml", buffer.Bytes(), os.FileMode(777)); err != nil {
		return err
	}
