...
then
  ...
  echo "kafka-util.sh consume-topic"
...
schemaRegistryIP={{ range $idx, $data := .SchemaRegistry -}}{{if eq $idx 0}}{{$data}}{{end}}{{- end }}
...
consume-topic)
  kafka-avro-console-consumer --bootstrap-server $brokerList --topic $2 --partition 0 --from-beginning --property schema.registry.url="http://$schemaRegistryIP:8081" --property print.key=true --property print.value=true
esac
