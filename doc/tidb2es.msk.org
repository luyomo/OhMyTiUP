...
workstation$ ./tiflow-glue/bin/cdc cli changefeed create --server http://172.82.11.79:9300 --changefeed-id=tidb2es-test --sink-uri="kafka://b-1.tidb2es.ux3vvn.c8.kafka.us-east-1.amazonaws.com:9092/topic-name?protocol=avro&replication-factor=3" --schema-registry=http://172.83.2.193:8081 --config /opt/kafka/source.toml
mysql$ MySQL [test]> create table test01(col01 bigint primary key auto_random, col02 int ); 
...
workstation$ cp aws-glue-schema-registry/avro-kafkaconnect-converter/target/dependency/* confluentinc-kafka-connect-elasticsearch-14.0.3/lib/
workstation$ cp aws-glue-schema-registry/avro-kafkaconnect-converter/target/schema-registry-kafkaconnect-converter-1.1.14.jar confluentinc-kafka-connect-elasticsearch-14.0.3/lib/
workstation$ mkdir tidb2es-sinkconnector-plugin
workstation$ cp confluentinc-kafka-connect-elasticsearch-14.0.3/lib/* tidb2es-sinkconnector-plugin/
workstation$ zip -r tidb2es-sinkconnector-plugin.zip tidb2es-sinkconnector-plugin/*
workstation$ aws s3 cp $(pwd)/tidb2es-sinkconnector-plugin.zip s3://ossinsight-data/kafka/
    #+END_SRC
