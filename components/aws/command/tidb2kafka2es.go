...
func newTiDB2Kafka2ESCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "tidb2kafka2es <sub_command>",
		Short: "Run commands for syncing the data to es from tidb through kafka",
	}

	cmd.AddCommand(
		newTiDB2Kafka2ESDeploy(),
...
func newTiDB2Kafka2ESDeploy() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.TiDB2Kafka2ESDeploy(clusterName, "ohmytiup-tidb2kafka2es", topoFile, opt, gOpt)
		},
	}
...
func newListTiDB2Kafka2ESCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.ListTiDB2Kafka2ESCluster(clusterName, "ohmytiup-tidb2kafka2es", opt)
		},
	}
...
func newDestroyTiDB2Kafka2ESCmd() *cobra.Command {
...
You can retain some nodes and roles data when destroy cluster, eg:
  ...
  $ tiup cluster destroy <cluster-name> --retain-node-data 172.16.13.12`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.DestroyTiDB2Kafka2ESCluster(clusterName, "ohmytiup-tidb2kafka2es", gOpt, destroyOpt, skipConfirm)
		},
	}
