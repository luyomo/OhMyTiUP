...
func newTiDB2AuroraCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "tidb2aurora <sub_command>",
		Short: "Run commands for tidb 2 aurora demo",
	}
...
func newListTiDB2AuroraCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.ListTiDB2AuroraCluster(clusterName, opt)
		},
	}
...
func newDestroyTiDB2AuroraCmd() *cobra.Command {
...
You can retain some nodes and roles data when destroy cluster, eg:
  ...
  $ tiup cluster destroy <cluster-name> --retain-node-data 172.16.13.12`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.DestroyTiDB2AuroraCluster(clusterName, gOpt, destroyOpt, skipConfirm)
		},
	}
...
/*
func test_subcommand() *cobra.Command {
	opt := manager.DeployOptions{
		IdentityFile: path.Join(utils.UserHome(), ".ssh", "id_rsa"),
	}
	cmd := &cobra.Command{
		Use:   "list_tidb2aurora <cluster-name>",
		Short: "List all clusters or cluster of aurora db",
		RunE: func(cmd *cobra.Command, args []string) error {
			shouldContinue, err := tui.CheckCommandArgsAndMayPrintHelp(cmd, args, 1)
			if err != nil {
