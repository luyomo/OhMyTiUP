...
func newPostgresCmd() *cobra.Command {
	...
	cmd.AddCommand(
		newPostgresDeploy(),
		newListPostgresCmd(),
		newDestroyPostgresCmd(),
	)
	return cmd
}

func newPostgresDeploy() *cobra.Command {
	opt := manager.PostgresDeployOptions{
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.PostgresDeploy(clusterName, topoFile, opt, postDeployHook, skipConfirm, gOpt)
		},
	}
...
func newListPostgresCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.ListPostgresCluster(clusterName, opt)
		},
	}
...
func newDestroyPostgresCmd() *cobra.Command {
...
You can retain some nodes and roles data when destroy cluster, eg:
  ...
  $ tiup cluster destroy <cluster-name> --retain-node-data 172.16.13.12`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.DestroyPostgresCluster(clusterName, gOpt, destroyOpt, skipConfirm)
		},
	}
