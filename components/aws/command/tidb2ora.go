...
func newTiDB2OraCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "tidb2ora <sub_command>",
		Short: "Run commands for tidb to oracle using TiDB binlog",
	}

	cmd.AddCommand(
		newTiDB2OraDeploy(),
		newListTiDB2OraCmd(),
		newDestroyTiDB2OraCmd(),
		//newSysbenchTiCDCCmd(),
...
func newTiDB2OraDeploy() *cobra.Command {
	opt := manager.TiDB2OraDeployOptions{
		IdentityFile: path.Join(utils.UserHome(), ".ssh", "id_rsa"),
	}
	cmd := &cobra.Command{
		Use:          "deploy <cluster-name> <topology.yaml>",
		Short:        "Deploy TiDB with pump/drainer and RDS Oracle for demo",
		Long:         "Deploy TiDB with pump/drainer and RDS Oracle to test the binlog data sync. SSH connection will be used to deploy files, as well as creating system users for running the service.",
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.TiDB2OraDeploy(clusterName, topoFile, opt, postDeployHook, skipConfirm, gOpt)
		},
	}
...
func newListTiDB2OraCmd() *cobra.Command {
	opt := manager.DeployOptions{
		IdentityFile: path.Join(utils.UserHome(), ".ssh", "id_rsa"),
	}
	cmd := &cobra.Command{
		Use:   "list <cluster-name>",
		Short: "List all clusters or cluster of tidb2ora resources",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.ListTiDB2OraCluster(clusterName, opt)
		},
	}
...
func newDestroyTiDB2OraCmd() *cobra.Command {
...
You can retain some nodes and roles data when destroy cluster, eg:
  ...
  $ tiup cluster destroy <cluster-name> --retain-node-data 172.16.13.12`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.DestroyTiDB2OraCluster(clusterName, gOpt, destroyOpt, skipConfirm)
		},
	}
...
//func newTiDB2OraScale() *cobra.Command {
//	opt := manager.TiDB2OraScaleOptions{
//		IdentityFile: path.Join(utils.UserHome(), ".ssh", "id_rsa"),
//	}
//	cmd := &cobra.Command{
...
//
//			return cm.TiDB2OraScale(clusterName, topoFile, opt, postDeployHook, skipConfirm, gOpt)
//		},
//	}
//
//	cmd.Flags().StringVarP(&opt.User, "user", "u", utils.CurrentUser(), "The user name to login via SSH. The user must has root (or sudo) privilege.")
