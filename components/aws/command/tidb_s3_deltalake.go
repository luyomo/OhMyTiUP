...
func newTiDB2S3DeltaLakeCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "tidb-s3-deltalake <sub_command>",
		Short: "Run commands for syncing the data to es from tidb through kafka",
	}

	cmd.AddCommand(
		newDeployTiDB2S3DeltaLakeCmd(),
		newListTiDB2S3DeltaLakeCmd(),
		newDestroyTiDB2S3DeltaLakeCmd(),
	)
	return cmd
}

func newDeployTiDB2S3DeltaLakeCmd() *cobra.Command {
	opt := manager.TiDB2Kafka2PgDeployOptions{
		IdentityFile: path.Join(utils.UserHome(), ".ssh", "id_rsa"),
	}
	cmd := &cobra.Command{
		Use:          "deploy <cluster-name> <topology.yaml>",
		Short:        "Deploy TiDB + kafka + ES",
		Long:         "Deploy an migration flow tidb - kafka - ES demo. SSH connection will be used to deploy files, as well as creating system users for running the service.",
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.TiDB2S3DeltaLakeDeploy(clusterName, TiDB2S3DELTALAKE, topoFile, opt, gOpt)
		},
	}
...
func newListTiDB2S3DeltaLakeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.ListS3DeltaLakeCluster(clusterName, TiDB2S3DELTALAKE, opt)
		},
	}
...
func newDestroyTiDB2S3DeltaLakeCmd() *cobra.Command {
...
You can retain some nodes and roles data when destroy cluster, eg:
  ...
  $ tiup cluster destroy <cluster-name> --retain-node-data 172.16.13.12`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cm.DestroyTiDB2Kafka2ESCluster(clusterName, TiDB2S3DELTALAKE, gOpt, destroyOpt, skipConfirm)
		},
	}
